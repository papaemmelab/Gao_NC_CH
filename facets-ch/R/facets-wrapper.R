...
preProcSample <- function(rcmat, ndepth=35, het.thresh=0.25, snp.nbhd=250, cval=25, deltaCN=0, gamma=0.25,
                          gbuild=c("hg19", "hg38", "hg18", "mm9", "mm10", "udef"), ugcpct=NULL, 
    ...
    if (!is.null(het_bias)) {
        dmat = dmat %>% 
            ...
            left_join(het_bias, by = "snp_index") %>% 
            mutate(valor0 = valor) %>%
            # should have used natural log instead of log2
            mutate(valor = valor0 - bias_median/1.5) %>%
            mutate(het = ifelse(is.na(valor), 0, het))
    }
    
    tmp <- segsnps(dmat, cval, hetscale, deltaCN, gamma)
